...
function DetailRow({ label, value }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 10, padding: '8px 0', borderBottom: '1px solid var(--border)', fontSize: 10.8 }}>
  ...
  return (
    <Modal open={open} title="User Details" onClose={onClose} maxWidth={640} footer={footer}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18, paddingBottom: 14, borderBottom: '2px solid var(--brand-light)' }}>
        <div style={{ width: 54, height: 54, borderRadius: '50%', background: 'var(--brand)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18.3, fontWeight: 700 }}>
          {(user.firstName || user.email || 'U')[0].toUpperCase()}
        </div>
        <div>
          <div style={{ fontSize: 14.9, fontWeight: 600, color: 'var(--text)' }}>{fullName}</div>
          <div style={{ fontSize: 10.8, color: 'var(--muted)' }}>{user.email}</div>
      ...
      <div className="card-title" style={{ fontSize: 11.6, marginBottom: 8 }}>Personal Information</div>
      ...
      <div className="card-title" style={{ fontSize: 11.6, margin: '16px 0 8px' }}>Module Permissions</div>
        ...
        {PERMISSION_MODULES.map((mod) => {
          ...
          return (
            <div key={mod.key} style={{ marginBottom: 6 }}>
              ...
              {granted.length === 0
                ? <span style={{ color: 'var(--muted)', fontSize: 10 }}>No access</span>
                : <span style={{ color: 'var(--success)', fontSize: 10 }}>{granted.map((a) => ACTION_LABELS[a] || a).join(', ')}</span>}
            </div>
          );
        })}
      </div>
